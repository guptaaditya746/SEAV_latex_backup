...
ROS 2 uses a modular architecture composed of nodes, each encapsulating specific functionalities such as sensor processing, localization, and control. Nodes interact exclusively through clearly defined interfaces:
\begin{itemize}
    \item Topics
    \begin{itemize}
        \item Provide asynchronous data communication channels ideal for continuous sensor data streams, such as LiDAR point clouds and odometry updates.
    \end{itemize}

    \item Services
    \begin{itemize}
        \item Enable synchronous request-response communication, useful for discrete commands or status checks like initiating sensor calibration.
    \end{itemize}

    \item Actions
    \begin{itemize}
        \item Support goal-oriented asynchronous tasks with feedback loops, beneficial for complex operations such as navigation or docking maneuvers.
    \end{itemize}
...
The ROS 2 Navigation Stack (Nav2) provides a robust, modular, and widely-adopted framework essential for autonomous navigation tasks. It integrates localization, global and local planning, obstacle avoidance, and recovery strategies into a cohesive architecture. Utilizing a behavior tree approach, Nav2 orchestrates these functionalities dynamically, offering flexibility and adaptability crucial for real-world autonomous vehicle operations.
...
\begin{itemize}
    ...
    \item Execute obstacle avoidance using costmaps (`global\_costmap`, `local\_costmap`).
    \item Enable smooth motion control via `cmd\_vel` commands sent to the base controller.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Dynamic Replanning}: 
    \begin{itemize}
        \item Nav2 regularly updates the global path to adapt to new obstacles or environmental changes, typically on the order of seconds. This ensures the vehicle can continuously adjust its path in response to dynamic conditions without human intervention.
    \end{itemize}
